  ...
  template: `
      <template v-if="isDataAvailable()">
        ...
        <template v-if="showOiSeries" >
          <div class="oiSeries" >
              <apex-oi-series-chart :symbol="symbol" :time="Date.now()" :strikePrice="ATM+strikeInterval*2">Place for OI Series Line Chart</apex-oi-series-chart>
              <apex-oi-series-chart :symbol="symbol" :time="Date.now()" :strikePrice="ATM+strikeInterval">Place for OI Series Line Chart</apex-oi-series-chart>
          </div>
          <div class="oiSeries">
            <apex-oi-series-chart :symbol="symbol" :time="Date.now()" :strikePrice="ATM">Place for OI Series Line Chart</apex-oi-series-chart>
        ...
        <template v-if="showOptionChain">
          ...
          <div class="chainTabContainer">
            <table id="optionAnalyzer" class="report">
              ...
              <tr v-for="strikes in store.getStrikesDetails(symbol)" :class="{atm: strikes.strikePrice == ATM}">
                <td :class="{itm: strikes.strikePrice < ATM}"><span v-if="strikes.CE.greeks" > {{ parseFloat(strikes.CE.greeks.delta).toFixed(3)  }} </span> </td>
                <td :class="{itm: strikes.strikePrice < ATM}">{{ parseFloat(strikes.CE.impliedVolatility).toFixed(2)  }}</td>
                <td :class="{ red: strikes.CE.changeinOpenInterest < 0, itm: strikes.strikePrice < ATM, highOIchgCall: store.getTotals(symbol).CE.highOIchgStrike == strikes.strikePrice }">{{ Number(strikes.CE.changeinOpenInterest * oiMultiplier).toLocaleString() }}</td>
                <td :class="{itm: strikes.strikePrice < ATM, highOICall: store.getTotals(symbol).CE.highOIStrike == strikes.strikePrice, secondHigh: store.getTotals(symbol).CE.secondHighOIStrike == strikes.strikePrice, thirdHigh: store.getTotals(symbol).CE.thirdHighOIStrike == strikes.strikePrice}">{{ Number(strikes.CE.openInterest * oiMultiplier).toLocaleString() }}</td>
                <td :class="{itm: strikes.strikePrice < ATM, highVolumeCall: store.getTotals(symbol).CE.highVolStrike == strikes.strikePrice, secondHigh: store.getTotals(symbol).CE.secondHighVolStrike == strikes.strikePrice, thirdHigh: store.getTotals(symbol).CE.thirdHighVolStrike == strikes.strikePrice}">{{ Number(strikes.CE.totalTradedVolume * oiMultiplier).toLocaleString() }}</td>
                <td :class="{itm: strikes.strikePrice < ATM, priceRed: strikes.CE.change < 0, priceGreen: strikes.CE.change > 0 }">{{ parseFloat(strikes.CE.change).toFixed(2) }}</td>
                <td :class="{itm: strikes.strikePrice < ATM}">{{ parseFloat(strikes.CE.premium).toFixed(2) }} <span class="premium">{{parseFloat(strikes.CE.premiumPercent).toFixed(2)}}</span></td>
                <td :class="{itm: strikes.strikePrice < ATM}">{{ parseFloat(strikes.CE.actualValue).toFixed(2) }}</td>
                <td :class="{itm: strikes.strikePrice < ATM, discount: strikes.CE.premium < 0}">{{ parseFloat(strikes.CE.lastPrice).toFixed(2) }}</td>

                <td class="strikePrice">{{ strikes.strikePrice }}</td>

                <td :class="{itm: strikes.strikePrice > ATM, discount: strikes.PE.premium < 0 }">{{ parseFloat(strikes.PE.lastPrice).toFixed(2) }}</td>
                <td :class="{itm: strikes.strikePrice > ATM}">{{ parseFloat(strikes.PE.actualValue).toFixed(2) }}</td>
                <td :class="{itm: strikes.strikePrice > ATM}">{{ parseFloat(strikes.PE.premium).toFixed(2) }} <span class="premium"> {{parseFloat(strikes.PE.premiumPercent).toFixed(2)}}</span></td>
                <td :class="{itm: strikes.strikePrice > ATM, priceRed: strikes.PE.change < 0, priceGreen: strikes.PE.change > 0 }">{{ parseFloat(strikes.PE.change).toFixed(2) }}</td>
                <td :class="{itm: strikes.strikePrice > ATM, highVolumePut: store.getTotals(symbol).PE.highVolStrike == strikes.strikePrice, secondHigh: store.getTotals(symbol).PE.secondHighVolStrike == strikes.strikePrice, thirdHigh: store.getTotals(symbol).PE.thirdHighVolStrike == strikes.strikePrice}">{{ Number(strikes.PE.totalTradedVolume * oiMultiplier).toLocaleString() }}</td>
                <td :class="{itm: strikes.strikePrice > ATM, highOIPut: store.getTotals(symbol).PE.highOIStrike == strikes.strikePrice, secondHigh: store.getTotals(symbol).PE.secondHighOIStrike == strikes.strikePrice, thirdHigh: store.getTotals(symbol).PE.thirdHighOIStrike == strikes.strikePrice}">{{ Number(strikes.PE.openInterest * oiMultiplier).toLocaleString() }}</td>
                <td :class="{ red: strikes.PE.changeinOpenInterest < 0, itm: strikes.strikePrice > ATM, highOIchgPut: store.getTotals(symbol).PE.highOIchgStrike == strikes.strikePrice }">{{ Number(strikes.PE.changeinOpenInterest * oiMultiplier).toLocaleString() }}</td>
                <td :class="{itm: strikes.strikePrice > ATM}">{{ parseFloat(strikes.PE.impliedVolatility).toFixed(2)  }}</td>
                <td :class="{itm: strikes.strikePrice > ATM}"><span v-if="strikes.CE.greeks" > {{ parseFloat(strikes.PE.greeks.delta).toFixed(3) }} </span> </td>
              </tr>
              <tr class="totals">
                <td colspan=2>&nbsp;</td>
                <td>&Sigma; OI Chg <hr />
